...
// Postcondtion: A new populated Question exists
template <class T>
Question<T>::Question(const Question& questionCopy){

  m_subject = questionCopy.m_subject;
  m_question = questionCopy.m_question;
  m_datatype = questionCopy.m_datatype;
  m_difficulty = questionCopy.m_difficulty;
  m_answer = questionCopy.m_answer;
  m_isAnswered = questionCopy.m_isAnswered;
...
template<class Y> //As friend function, has to use different templated variable
ostream& operator<< (ostream& output, Question<Y>& questionCopy){
  output << questionCopy.m_question;
  return output;
}
